...
class PersonDossier(BaseModel):
    """
    Structured research dossier of verifiable, citable facts about a cinema professional
    (Actor, Director, Writer, Crew). Deliberately excludes synthesized judgments (chemistry
    scores, sentiment, working-style characterizations) -- those are computed by Raphael from
    these facts in a separate downstream step, not requested from the research call itself.
    """
