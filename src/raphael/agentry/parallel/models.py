...
class PersonDossier(BaseModel):
    ...
    Every field beyond name is optional: this schema is shared by both a shallow
    candidate-search pass (which can only plausibly fill in a few fields from
    search snippets) and a deep-research pass (which can fill in the rest) --
    a partially-filled dossier is expected, not an error.
    """
    name: str = Field(description="Full name of the person")
    primary_roles: list[str] = Field(default_factory=list, description="Primary roles (e.g. Actor, Director, Cinematographer)")
    bio_summary: Optional[str] = Field(default=None, description="Brief biographical and career summary")
